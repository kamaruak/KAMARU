...
Asena.addCommand({ pattern: 'insta ?(.*)', fromMe: false, desc: "Downloads from instagaram", dontAddCommandList: true }, async (message, match) => { 
    ...
    if (type === 'image') return await message.sendMessage(data, MessageType.image, { caption: "```🌹✮𝕶𝖆ͥ𝖒ᷞ𝖆ͦ𝖗ͮ𝖚ͤ║𝕬𝖐ͧ✮🌹```", quoted: message.data })
    if (type === 'video') return await message.sendMessage(data, MessageType.video, { caption: "```🌹✮𝕶𝖆ͥ𝖒ᷞ𝖆ͦ𝖗ͮ𝖚ͤ║𝕬𝖐ͧ✮🌹```", quoted: message.data })
});
